...
// Parameters:
// - autoInit: Automatically build the initializer and respond to external UserDefaults changes when
// ignoreExternalChanges is false
// - ignoreExternalChanges: Ignore notifications from external modifications. When true, the
// instance will only respond to its own data modifications
// - suiteName: The suite name for UserDefaults
// - prefix: Prefix for UserDefaults keys. Note: It cannot contain the '.' character
// - observeFirst: Observation priority mode. When enabled (set to true), only properties explicitly
// marked with `@DefaultsBacked` will correspond to UserDefaults, while others will be treated as
// ObservableOnly. The default value is false
// If a parameter is set in both the macro and the initializer, the value in the initializer takes
// precedence
@attached(
    member,
