...
func nullableWithObservableDefaults() async throws {
    // Clean UserDefaults
...
func nullableHelperMethods() async throws {
    ...
    // Test map
    let mappedNames = nullableFromValue.map(\.name)
    #expect(mappedNames.value == "Alice")

    let mappedNil = nullableFromNil.map(\.name)
    #expect(mappedNil.value == nil)
}

@Test("Nullable should support pattern matching")
func nullablePatternMatching() async throws {
...
func nullableEquatable() async throws {
